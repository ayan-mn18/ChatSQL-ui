...
function DatabaseNode({ delay = 0, size = 'md' }: { delay?: number; size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-emerald-500/20 rounded-lg blur-xl animate-pulse" />
      <div className="relative w-full h-full bg-gradient-to-br from-slate-800 to-slate-900 rounded-lg border border-emerald-500/30 flex items-center justify-center shadow-[0_0_30px_rgba(16,185,129,0.2)]">
        <Database className="w-1/2 h-1/2 text-emerald-400" />
      </div>
    </div>
  );
}
...
function ConnectionLine({ className = '' }: { className?: string }) {
  return (
    <div className={`absolute ${className}`}>
      <div className="w-full h-0.5 bg-gradient-to-r from-transparent via-emerald-500/50 to-transparent animate-pulse" />
    </div>
  );
}
    ...
    <svg className="absolute inset-0 w-full h-full opacity-5" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <pattern id="circuit" x="0" y="0" width="100" height="100" patternUnits="userSpaceOnUse">
          <path d="M0 50 H40 M60 50 H100" stroke="currentColor" strokeWidth="1" fill="none" className="text-emerald-400" />
          <path d="M50 0 V40 M50 60 V100" stroke="currentColor" strokeWidth="1" fill="none" className="text-emerald-400" />
          <circle cx="50" cy="50" r="4" fill="currentColor" className="text-emerald-400" />
          <circle cx="0" cy="50" r="2" fill="currentColor" className="text-emerald-400" />
          <circle cx="100" cy="50" r="2" fill="currentColor" className="text-emerald-400" />
          <circle cx="50" cy="0" r="2" fill="currentColor" className="text-emerald-400" />
          <circle cx="50" cy="100" r="2" fill="currentColor" className="text-emerald-400" />
        </pattern>
      </defs>
...
function FeatureCard({
  ...
  const colorClasses = {
    cyan: 'from-emerald-500/20 to-emerald-500/5 border-emerald-500/30 hover:border-emerald-400/50 text-emerald-400',
    purple: 'from-green-500/20 to-green-500/5 border-green-500/30 hover:border-green-400/50 text-green-400',
    emerald: 'from-emerald-500/20 to-emerald-500/5 border-emerald-500/30 hover:border-emerald-400/50 text-emerald-400',
    amber: 'from-teal-500/20 to-teal-500/5 border-teal-500/30 hover:border-teal-400/50 text-teal-400'
  };
...
LIMIT 10;`;
  ...
  return (
    <div className="relative">
      <div className="absolute -inset-4 bg-gradient-to-r from-emerald-500/20 via-green-500/20 to-emerald-500/20 rounded-3xl blur-2xl opacity-50" />
...
function AIChatPreview() {
  return (
    <div className="relative">
      <div className="absolute -inset-4 bg-gradient-to-r from-green-500/20 via-emerald-500/20 to-green-500/20 rounded-3xl blur-2xl opacity-50" />
      <div className="relative bg-slate-900/90 rounded-2xl border border-slate-700/50 overflow-hidden backdrop-blur-xl">
        <div className="p-4 space-y-4">
          {/* User message */}
          <div className="flex justify-end">
            <div className="max-w-[80%] bg-emerald-600/20 border border-emerald-500/30 rounded-2xl rounded-br-md px-4 py-3">
          ...
          <div className="flex justify-start">
            <div className="max-w-[80%] bg-slate-800/50 border border-slate-600/30 rounded-2xl rounded-bl-md px-4 py-3">
              ...
              <code className="text-xs text-emerald-300 bg-slate-900/50 px-2 py-1 rounded">SELECT name, SUM(total)...</code>
            </div>
          </div>
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
      {stats.map((stat, i) => (
        <div
          key={i}
          className="relative group p-6 bg-slate-900/50 rounded-2xl border border-slate-700/50 hover:border-emerald-500/30 transition-all duration-300"
        >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <nav className="hidden md:flex items-center gap-8">
              <a href="#features" className="text-gray-400 hover:text-emerald-400 transition-colors text-sm font-medium">Features</a>
              <a href="#preview" className="text-gray-400 hover:text-emerald-400 transition-colors text-sm font-medium">Preview</a>
              <a href="#stats" className="text-gray-400 hover:text-emerald-400 transition-colors text-sm font-medium">Stats</a>
              <a
      ...
      <section className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 text-center">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800/50 border border-slate-700/50 text-sm mb-8 backdrop-blur-sm">
            ...
            <span className="text-emerald-400 font-medium">AI-Powered DB Visualizer</span>
          </div>

          {/* Main heading */}
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold mb-8 leading-tight tracking-tight">
            <span className="block text-white mb-2">The Ultimate AI-Powered</span>
            <span className="block bg-gradient-to-r from-emerald-400 via-green-400 to-emerald-400 bg-clip-text text-transparent animate-gradient bg-300%">
              Database Visualizer
            </span>
          </h1>
          ...
          <p className="text-xl text-gray-400 max-w-3xl mx-auto mb-12 leading-relaxed">
            Connect to any database, run complex queries, set up triggers, and visualize insights instantly using natural language.
            Reliable, strong, and built for modern data teams.
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-3 px-8 py-4 rounded-full text-lg font-semibold bg-slate-800/50 border border-slate-700/50 hover:bg-slate-800 hover:border-emerald-500/30 transition-all duration-300 backdrop-blur-sm"
            >
        ...
        <div className="relative max-w-7xl mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">
              ...
              <span className="block bg-gradient-to-r from-emerald-400 to-green-500 bg-clip-text text-transparent mt-2">
                Modern Data Teams
              </span>
            </h2>
      ...
      <section className="py-32 relative">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <div className="relative p-12 rounded-3xl bg-gradient-to-br from-slate-800/50 to-slate-900/50 border border-slate-700/50 overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 w-64 h-64 bg-green-500/10 rounded-full blur-[100px]" />
            ...
            <div className="relative">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link
                  to="/chat"
                  className="inline-flex items-center justify-center gap-3 px-8 py-4 rounded-full text-lg font-semibold bg-gradient-to-r from-emerald-500 to-emerald-600 text-white hover:from-emerald-400 hover:to-emerald-500 transition-all duration-300 shadow-[0_0_30px_rgba(16,185,129,0.3)]"
                >
                  Get Started Free
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center justify-center gap-3 px-8 py-4 rounded-full text-lg font-semibold border border-slate-600 hover:border-emerald-500/50 hover:bg-slate-800/50 transition-all duration-300"
                >
                  <Github className="w-5 h-5" />
                  Star on GitHub
                </a>
